...
//        }
    ...
    public long deleteLastAudio(){
        int index = mVideoList.size() - 1;
        MediaInfo mediaInfo = null;
        if (index >= 0) {
            mediaInfo = mVideoList.get(index);
        }
...
//        retriever.setDataSource(mActivity, Uri.fromFile(mVideoList.get(index)));
...
//        mCameraRenderer.requestRender();
    ...
    private void mergeVideos(String file1,String file2) {
        ArrayList<EpVideo> epVideos =  new  ArrayList<>();
        epVideos.add(new EpVideo (file2)); // Video 1
        epVideos.add(new EpVideo (file1)); // Video 2
        String fileOutput = generateOutputPath();
        ...
        EpEditor.merge(epVideos, outputOption, new  OnEditorListener() {
            ...
            public  void  onSuccess () {
                ...
                Log.d("test_test",fileOutput);
                onOpenVideoStitchPage(fileOutput);
            }

            @Override
            public  void  onFailure () {
                Log.d("test_test",""+"Failed");
            }

            @Override
            public  void  onProgress ( float  progress ) {
                // Get processing progress here
                Log.d("test_test",""+progress);
            }
        });

    }

    public void stitchAndEdit(String stitchPath){
    ...
    public void mergeAndEdit() {
            ...
            if (mVideoList.size() == 1) {
                ...
                Intent intent = null;
                try {
                    intent = new Intent(mActivity, Class.forName("com.digitok.VideoEditing.VideoEditingActivity"));
                }
                ...
                intent.putExtra("fromKey", "");
                intent.putExtra("fromValue", "");
                if(mMusicPath==null || mMusicPath==""){
                    mMusicPath="";
                }
                intent.putExtra("audiofile", ""+mMusicPath);
                intent.putExtra("IsFromRecording", "1");
                mActivity.startActivity(intent);
                mActivity.finish();
            } else {
                getTarget().showConcatProgressDialog();
                ...
                for (MediaInfo info : mVideoList) {
                    if (info != null && !TextUtils.isEmpty(info.getFileName())) {
                        videos.add(info.getFileName());
                        Log.d("filename",info.getFileName());
                    }
                }
                String finalPath = generateOutputPath();
                mCommandEditor.execCommand(CainCommandEditor.concatVideo(mActivity, videos, finalPath),
    ...
    public void onOpenVideoStitchPage(String path) {

            Log.d("test_test","Inside function");
            Intent intent = null;
            try {
                intent = new Intent(mActivity, Class.forName("com.cgfay.video.activity.VideoCutActivity"));
            }
            catch (ClassNotFoundException e) {
                e.printStackTrace();
            }
